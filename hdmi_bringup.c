...
int main(void)
{
    ...
    /* === PHASE 5: PHY sequence (Linux inno_hdmi_phy_rk3399 order) === */
    /*
     * Correct sequence from Linux phy-rockchip-inno-hdmi.c:
     *   pre-init: write PHY reg 0x02 = 0 (clear PDATAEN)
     *   1. PHY_CONF0 = 0xE2  (PDZ|ENTMDS|SPARECTRL|SELDATAENPOL, no TXPWRON)
     *   2. Wait 5ms  (PHY analog powers up)
     *   3. Write MPLL via I2C  (PHY must be powered for MPLL to latch)
     *   4. PHY_CONF0 = 0xF2  (add GEN2_TXPWRON)
     *   5. Assert MC_PHYRSTZ = 0x00
     *   6. Wait 100us
     *   7. Release MC_PHYRSTZ = 0x01
     *   8. Poll PHY_STAT0
     */
    printf("\n[5] PHY init (correct Linux sequence)\n");

    /* I2C divider setup first */
    hw(0x3029, 0x17);
    hw(0x3027, 0xff);
    hw(0x3028, 0xff);
    ...
    /* Step 1: Power up PHY analog (no TXPWRON yet) */
    hw(0x3000, 0xe2);
    usleep(5000);
    printf("    step1: PHY_CONF0=0x%02x (want 0xe2)\n", hr(0x3000));

    /* Step 3: Write MPLL with PHY powered up */
    ...
    hw(0x3000, 0xf2);
    usleep(2000);
    printf("    step4: PHY_CONF0=0x%02x (want 0xf2)\n", hr(0x3000));

    /* Steps 5-7: Assert then release PHY reset */
    hw(0x4005, 0x00);
    usleep(100);
    hw(0x4005, 0x01);
    usleep(3000);
    printf("    step7: MC_PHYRSTZ=%02x PHY_STAT0=%02x\n",
        hr(0x4005), hr(0x3004));

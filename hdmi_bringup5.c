...
int main(void)
{
    ...
    if (HR(0x3004) & 0x10) {
        uint32_t fb_pa = 0;
        /* Get framebuffer PA from rkfb if available */
        {
            int rfd = open("/dev/rkfb0", O_RDWR);
            if (rfd >= 0) {
                struct { uint32_t w,h,bpp,stride; uint64_t size,pa; } info;
                if (ioctl(rfd, 0x52000000|((int)sizeof(info)<<16)|0, &info)==0)
                    fb_pa = (uint32_t)(info.pa & 0xffffffff);
                close(rfd);
            }
        }
        if (!fb_pa) fb_pa = 0xf5800000; /* fallback */

        printf("Holding display active (fb_pa=0x%08x). Press Ctrl+C to exit.\n", fb_pa);
        for (;;) {
            hold_regs();
            HW(0x4002, 0xdf);
            HW(0x4001, 0x74);
            HW(0x4005, 0x01);
            /* Hold VOP 720p timing and WIN0 */
            {
                uint32_t sc = vop_r(0x0008);
                sc &= ~(1u<<11); sc |= (1u<<1);
                vop_w(0x0008, sc);
            }

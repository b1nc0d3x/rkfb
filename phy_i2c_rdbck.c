...
static int phy_read(uint8_t reg, uint16_t *val)
{
    ...
    hdmi[0x3020] = 0x69;
    hdmi[0x3021] = reg;
    hdmi[0x3026] = 0x08;   /* RD operation - bit3, NOT bit6 */
    for (i=0; i<200; i++) {
        usleep(500);
        st = hdmi[0x3027];
        if (st & 0x02) {
            hdmi[0x3027] = 0x02;
            *val = ((uint16_t)hdmi[0x3024] << 8) | hdmi[0x3025];
            return 0;
        }
        if (st & 0x08) { hdmi[0x3027]=0x08; return -1; }
    }
    return -2; /* timeout */
}
...
static int phy_write(uint8_t reg, uint16_t val)
{
    ...
    hdmi[0x3020] = 0x69;
    hdmi[0x3021] = reg;
    hdmi[0x3022] = (val>>8)&0xff;
    hdmi[0x3023] = val&0xff;
    hdmi[0x3026] = 0x10;
    for (i=0; i<200; i++) {
        usleep(500);
        st = hdmi[0x3027];
        if (st & 0x02) { hdmi[0x3027]=0x02; return 0; }
        if (st & 0x08) { hdmi[0x3027]=0x08; return -1; }
    }
    return -2;
}
...
int main(void)
{
    ...
    hdmi[0x3000] = 0xf2;
    usleep(2000);
    hdmi[0x4005] = 0x00; usleep(100);
    hdmi[0x4005] = 0x01; usleep(5000);
